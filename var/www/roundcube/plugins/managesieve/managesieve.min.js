...
parent.rcmail.managesieve_dialog_resize(rcmail.gui_objects.sieveform),$('input[type="text"]:first',rcmail.gui_objects.sieveform).focus(),$('textarea[data-type="list"]',rcmail.gui_objects.sieveform).each(function(){smart_field_init(this)})):rcmail.enable_command("plugin.managesieve-add","plugin.managesieve-setadd",!rcmail.env.sieveconnerror);var b=rcmail;a=rcmail.env.currentset;rcmail.gui_objects.filterslist&&(rcmail.filters_list=new rcube_list_widget(rcmail.gui_objects.filterslist,{multiselect:!1,
draggable:!0,keyboard:!1}),rcmail.filters_list.addEventListener("select",function(a){b.managesieve_select(a)}),rcmail.filters_list.addEventListener("dragstart",function(a){b.managesieve_dragstart(a)}),rcmail.filters_list.addEventListener("dragend",function(a){b.managesieve_dragend(a)}),rcmail.filters_list.row_init=function(a){a.obj.onmouseover=function(){b.managesieve_focus_filter(a)};a.obj.onmouseout=function(){b.managesieve_unfocus_filter(a)}},rcmail.filters_list.init(),rcmail.filters_list.focus());
rcmail.gui_objects.filtersetslist&&(rcmail.filtersets_list=new rcube_list_widget(rcmail.gui_objects.filtersetslist,{multiselect:!1,draggable:!1,keyboard:!1}),rcmail.filtersets_list.addEventListener("select",function(a){b.managesieve_setselect(a)}),rcmail.filtersets_list.init(),rcmail.filtersets_list.focus(),null!=a&&(a=rcmail.managesieve_setid(a),rcmail.filtersets_list.shift_start=a,rcmail.filtersets_list.highlight_row(a,!1)),a=rcmail.filtersets_list.rowcount,rcmail.enable_command("plugin.managesieve-set",
!0),rcmail.enable_command("plugin.managesieve-setact","plugin.managesieve-setget",a),rcmail.enable_command("plugin.managesieve-setdel",1<a),$("tr",rcmail.gui_objects.filtersetslist).each(function(a,d){b.managesieve_fixdragend(d)}))}rcmail.gui_objects.sieveform&&rcmail.env.rule_disabled&&$("#disabled").attr("checked",!0)});rcube_webmail.prototype.managesieve_add=function(){this.load_managesieveframe();this.filters_list.clear_selection()};
...
rcube_webmail.prototype.managesieve_updatelist=function(a,b){this.set_busy(!0);switch(a){case "del":var c=0,d=this.filters_list;d.remove_row(this.managesieve_rowid(b.id));d.clear_selection();this.show_contentframe(!1);this.enable_command("plugin.managesieve-del","plugin.managesieve-act",!1);$("tr",this.filters_list.list).each(function(){"none"==this.style.display?$(this).detach():$(this).attr("id","rcmrow"+c++).unbind()});d.init();break;case "update":var e=$("#rcmrow"+this.managesieve_rowid(b.id));
b.name&&$("td",e).text(b.name);b.disabled?e.addClass("disabled"):e.removeClass("disabled");$("#disabled",$("iframe").contents()).prop("checked",b.disabled);break;case "add":d=this.filters_list;e=$('<tr><td class="name"></td></tr>');$("td",e).text(b.name);e.attr("id","rcmrow"+b.id);b.disabled&&e.addClass("disabled");d.insert_row(e.get(0));d.highlight_row(b.id);this.enable_command("plugin.managesieve-del","plugin.managesieve-act",!0);break;case "list":var f,g,d=this.filters_list;b.clear&&d.clear();
for(c in b.list)g=b.list[c],e=document.createElement("TR"),f=document.createElement("TD"),$(f).text(g.name),f.className="name",e.id="rcmrow"+g.id,g["class"]&&(e.className=g["class"]),e.appendChild(f),d.insert_row(e);b.set?d.highlight_row(b.set):this.enable_command("plugin.managesieve-del","plugin.managesieve-act",!1);break;case "setact":f=this.managesieve_setid(b.name);e=$("#rcmrow"+f);b.active?(b.all&&$("tr",this.gui_objects.filtersetslist).addClass("disabled"),e.removeClass("disabled")):e.addClass("disabled");
break;case "setdel":f=this.managesieve_setid(b.name);this.filtersets_list.remove_row(f);this.filters_list.clear();this.show_contentframe(!1);this.enable_command("plugin.managesieve-setdel","plugin.managesieve-setact","plugin.managesieve-setget",!1);delete this.env.filtersets[f];break;case "setadd":f="S"+(new Date).getTime(),d=this.filtersets_list,e=$('<tr class="disabled"><td class="name"></td></tr>'),$("td",e).text(b.name),e.attr("id","rcmrow"+f),this.env.filtersets[f]=b.name,d.insert_row(e.get(0)),
b.index!=d.rowcount-1&&(e.detach(),g=$("tr:visible",d.list).get(b.index),e.insertBefore(g)),d.select(f),this.managesieve_fixdragend(e)}this.set_busy(!1)};
...
rcube_webmail.prototype.managesieve_unfocus_filter=function(a){this.drag_active&&($(a.obj).removeClass("filtermoveup filtermovedown"),this.drag_filter_target=null)};rcube_webmail.prototype.managesieve_save=function(){if(parent.rcmail&&parent.rcmail.filters_list&&"filtersetform"!=this.gui_objects.sieveform.name){var a=parent.rcmail.filters_list.get_single_selection();null!=a&&(this.gui_objects.sieveform.elements._fid.value=parent.rcmail.filters_list.rows[a].uid)}this.gui_objects.sieveform.submit()};
...
rcube_webmail.prototype.managesieve_actionfill=function(a,b,c){if(""!=a){var d=document.getElementById("actions"),e=document.createElement("div");this.managesieve_insertrow(d,e,c);e.className="actionrow";e.setAttribute("id","actionrow"+b);e.innerHTML=a;this.managesieve_formbuttons(d)}};
...
function rule_header_select(a){var b=document.getElementById("header"+a),c=document.getElementById("rule_size"+a),d=document.getElementById("rule_op"+a),e=document.getElementById("custom_header"+a+"_list"),f=document.getElementById("rule_mod"+a),g=document.getElementById("rule_trans"+a),l=document.getElementById("rule_comp"+a),m=document.getElementById("rule_date_part"+a),n=document.getElementById("rule_date_header_div"+a),k=b.value;"size"==k?(c.style.display="inline",$.each([d,e,f,g,l],function(){this.style.display=
"none"})):(e.style.display="..."!=k?"none":"inline-block",c.style.display="none",d.style.display="inline",l.style.display="",f.style.display="body"==k||"currentdate"==k||"date"==k?"none":"block",g.style.display="body"==k?"block":"none");m&&(m.style.display="currentdate"==k||"date"==k?"inline":"none");n&&(n.style.display="date"==k?"":"none");rule_op_select(d,a,k);rule_mod_select(a,k);b.style.width="..."==k?"40px":""}
...
function smart_field_init(a){var b=a.id+"_list",c=$('<span class="listarea"></span>'),d=a.value?a.value.split("\n"):[""];$("#"+b).length||($.each(d,function(b,d){c.append(smart_field_row(d,a.name,b,$(a).data("size")))}),c.attr("id",b),a=$(a),a.attr("disabled")&&c.hide(),a.after(c),a.hasClass("error")&&(c.addClass("error"),rcmail.managesieve_tip_register([[b,a.data("tip")]])))}
...
return c}
rcube_webmail.prototype.managesieve_tip_register=function(a){var b,c=parent.rcmail,d=c?parent.rcmail.env.ms_tip_layer:rcmail.env.ms_tip_layer;for(b in a)$("#"+a[b][0]).data("tip",a[b][1]).bind("mouseenter",function(a){a=$(this);var b=a.offset(),g=b.left,l=b.top-12,m=a.width();c&&(b=$("mail"==rcmail.env.task?"#sievefilterform > iframe":"#filter-box",parent.document).offset(),l+=b.top,g+=b.left);d.html(a.data("tip"));l-=d.height();d.css({left:g,top:l,minWidth:m-2+"px"}).show()}).bind("mouseleave",function(a){d.hide()})};
rcube_webmail.prototype.managesieve_create=function(){if(rcmail.env.sieve_headers&&rcmail.env.sieve_headers.length){var a,b,c={},d=$("#sievefilterform");d.length||(d=$('<div id="sievefilterform"></div>'),$("body").append(d));b="<fieldset><legend>"+this.gettext("managesieve.usedata")+"</legend><ul>";for(a in rcmail.env.sieve_headers)b+='<li><input type="checkbox" name="headers[]" id="sievehdr'+a+'" value="'+a+'" checked="checked" /><label for="sievehdr'+a+'">'+rcmail.env.sieve_headers[a][0]+":</label> "+
rcmail.env.sieve_headers[a][1]+"</li>";d.html(b+"</ul></fieldset>");c[this.gettext("managesieve.nextstep")]=function(){var a=$('input[name="headers[]"]:checked',d);if(a.length){var b=rcmail.get_task_url("mail"),b=rcmail.add_url(b,"_action","plugin.managesieve"),b=rcmail.add_url(b,"_framed",1);a.map(function(){var a=rcmail.env.sieve_headers[this.value];b=rcmail.add_url(b,"r["+this.value+"]",a[0]+":"+a[1])});a=$("<iframe>").attr({src:b,frameborder:0});d.empty().append(a).dialog("widget").resize();c=
{};c[rcmail.gettext("save")]=function(){$("iframe",d).get(0).contentWindow.rcmail.managesieve_save()};d.dialog("option","buttons",c)}else alert(rcmail.gettext("managesieve.nodata"))};d.dialog({modal:!1,resizable:!bw.ie6,closeOnEscape:!bw.ie6&&!bw.ie7,title:this.gettext("managesieve.newfilter"),close:function(){rcmail.managesieve_dialog_close()},buttons:c,minWidth:600,minHeight:300,height:250}).show();this.env.managesieve_dialog=d}};
