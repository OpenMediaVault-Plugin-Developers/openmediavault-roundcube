...
a.rowcount)this.select_all_mode=!1;b&&this.env.contentframe&&!a.multi_selecting&&!this.dummy_select?this.preview_timer=setTimeout(function(){l.msglist_get_preview()},this.dblclick_time):this.env.contentframe&&this.show_contentframe(!1)};this.msglist_click=function(a){a.multi_selecting||!this.env.contentframe||a.get_single_selection()||(a=this.get_frame_window(this.env.contentframe))&&0<=a.location.href.indexOf(this.env.blankpage)&&(this.preview_timer&&clearTimeout(this.preview_timer),this.preview_read_timer&&
clearTimeout(this.preview_read_timer),this.preview_timer=setTimeout(function(){l.msglist_get_preview()},this.dblclick_time))};this.msglist_dbl_click=function(a){this.preview_timer&&clearTimeout(this.preview_timer);this.preview_read_timer&&clearTimeout(this.preview_read_timer);(a=a.get_single_selection())&&this.env.mailbox==this.env.drafts_mailbox?this.open_compose_step({_draft_uid:a,_mbox:this.env.mailbox}):a&&this.show_message(a,!1,!1)};this.msglist_keypress=function(a){a.modkey!=CONTROL_KEY&&(a.key_pressed==
...
0,unread_children:d.unread_children?d.unread_children:0,parent_uid:d.parent_uid?d.parent_uid:0,selected:this.select_all_mode||this.message_list.in_selection(a),ml:d.ml?1:0,ctype:d.ctype,flags:d.extra_flags});var f,g,h,k="",l="",m=this.message_list;h=m.rows;var n=this.env.messages[a],p="message"+(d.seen?"":" unread")+(d.deleted?" deleted":"")+(d.flagged?" flagged":"")+(n.selected?" selected":""),u={cols:[],style:{},id:"rcmrow"+a};f="msgicon";null===this.env.status_col&&(f+=" status",d.deleted?f+=" deleted":
d.seen?0<d.unread_children&&(f+=" unreadchildren"):f+=" unread");d.answered&&(f+=" replied");d.forwarded&&(f+=" forwarded");n.selected&&!m.in_selection(a)&&m.selection.push(a);this.env.threading&&(n.depth?(k+='<span id="rcmtab'+a+'" class="branch" style="width:'+15*n.depth+'px;">&nbsp;&nbsp;</span>',h[n.parent_uid]&&!1===h[n.parent_uid].expanded||!(0!=this.env.autoexpand_threads&&2!=this.env.autoexpand_threads||h[n.parent_uid]&&h[n.parent_uid].expanded)?(u.style.display="none",n.expanded=!1):n.expanded=
!0,p+=" thread expanded"):n.has_children&&(void 0===n.expanded&&(1==this.env.autoexpand_threads||2==this.env.autoexpand_threads&&n.unread_children)&&(n.expanded=!0),l='<div id="rcmexpando'+a+'" class="'+(n.expanded?"expanded":"collapsed")+'">&nbsp;&nbsp;</div>',p+=" thread"+(n.expanded?" expanded":"")),d.unread_children&&d.seen&&!n.expanded&&(p+=" unroot"));k+='<span id="msgicn'+a+'" class="'+f+'">&nbsp;</span>';u.className=p;!bw.ie&&b.subject&&(h=d.mbox==this.env.drafts_mailbox?"_draft_uid":"_uid",
b.subject='<a href="./?_task=mail&_action='+(d.mbox==this.env.drafts_mailbox?"compose":"show")+"&_mbox="+urlencode(d.mbox)+"&"+h+"="+a+'" onclick="return rcube_event.cancel(event)" onmouseover="rcube_webmail.long_subject_title(this,'+(n.depth+1)+')">'+b.subject+"</a>");for(g in this.env.coltypes)f=this.env.coltypes[g],h={className:String(f).toLowerCase()},"flag"==f?(f=d.flagged?"flagged":"unflagged",f='<span id="flagicn'+a+'" class="'+f+'">&nbsp;</span>'):"attachment"==f?f=/application\/|multipart\/(m|signed)/.test(d.ctype)?
'<span class="attachment">&nbsp;</span>':/multipart\/report/.test(d.ctype)?'<span class="report">&nbsp;</span>':"&nbsp;":"status"==f?(f=d.deleted?"deleted":d.seen?0<d.unread_children?"unreadchildren":"msgicon":"unread",f='<span id="statusicn'+a+'" class="'+f+'">&nbsp;</span>'):"threads"==f?f=l:"subject"==f?(bw.ie&&(h.onmouseover=function(){rcube_webmail.long_subject_title_ex(this,n.depth+1)},bw.ie8&&(k="<span></span>"+k)),f=k+b[f]):f="priority"==f?0<d.prio&&6>d.prio?'<span class="prio'+d.prio+'">&nbsp;</span>':
"&nbsp;":b[f],h.innerHTML=f,u.cols.push(h);m.insert_row(u,e);e&&this.env.pagesize&&m.rowcount>this.env.pagesize&&(a=m.get_last_row(),m.remove_row(a),m.clear_selection(a))};this.set_list_sorting=function(a,b){$("#rcm"+this.env.sort_col).removeClass("sorted"+this.env.sort_order.toUpperCase());a&&$("#rcm"+a).addClass("sorted"+b);this.env.sort_col=a;this.env.sort_order=b};this.set_list_options=function(a,b,d,e){var f,g={};void 0===b&&(b=this.env.sort_col);d||(d=this.env.sort_order);if(this.env.sort_col!=
...
100),this.env.opened_extwin=!0);0<this.env.autocomplete_threads&&(h={threads:this.env.autocomplete_threads,sources:this.env.autocomplete_sources});this.init_address_input_events(b,h);for(var q in g)this.init_address_input_events($("[name='_"+g[q]+"']"),h);f||(this.set_caret_pos(e,this.env.top_posting?0:$(e).val().length),"select-one"==a.prop("type")&&this.change_identity(a[0]));if(window.localStorage){var a=this.local_storage_get_item("compose.index",[]),m;for(q=0;q<a.length;q++)if(m=a[q],formdata=
...
a,$("input[name='_draft_saveid']").val(a));this.remove_compose_data(this.env.compose_id)};this.auto_save_start=function(){this.env.draft_autosave&&(this.save_timer=setTimeout(function(){l.command("savedraft")},1E3*this.env.draft_autosave));!this.local_save_timer&&window.localStorage&&(this.compose_type_activity=this.compose_type_activity_last=0,$(document).bind("keypress",function(a){l.compose_type_activity++}),this.local_save_timer=setInterval(function(){l.compose_type_activity>l.compose_type_activity_last&&
(l.save_compose_form_local(),l.compose_type_activity_last=l.compose_type_activity)},5E3));this.busy=!1};this.compose_field_hash=function(a){var b,d,e,f="",g=["to","cc","bcc","subject"];for(d=0;d<g.length;d++)if(e=$('[name="_'+g[d]+'"]').val())f+=e+":";f=window.tinyMCE&&(b=tinyMCE.get(this.env.composebody))?f+b.getContent():f+$("[name='_message']").val();if(this.env.attachments)for(var h in this.env.attachments)f+=h;a&&(this.cmp_hash=f);return f};this.save_compose_form_local=function(){var a={session:this.env.session_id,
changed:(new Date).getTime()},b=!0;window.tinyMCE&&tinyMCE.get(this.env.composebody)&&tinyMCE.triggerSave();this.env.draft_id&&(a.draft_id=this.env.draft_id);this.env.reply_msgid&&(a.reply_msgid=this.env.reply_msgid);$("input, select, textarea",this.gui_objects.messageform).each(function(d,e){switch(e.tagName.toLowerCase()){case "input":if("button"==e.type||"submit"==e.type||"hidden"==e.type&&"_is_html"!=e.name)break;a[e.name]="checkbox"!=e.type||e.checked?$(e).val():"";""!=a[e.name]&&"hidden"!=e.type&&
(b=!1);break;case "select":a[e.name]=$("option:checked",e).val();break;default:a[e.name]=$(e).val(),""!=a[e.name]&&(b=!1)}});if(window.localStorage&&!b){var d=this.local_storage_get_item("compose.index",[]),e=this.env.compose_id;0>$.inArray(e,d)&&d.push(e);this.local_storage_set_item("compose."+e,a,!0);this.local_storage_set_item("compose.index",d)}};this.restore_compose_form=function(a,b){var d=this.local_storage_get_item("compose."+a,!0);d&&"object"==typeof d&&($.each(d,function(a,b){if("_"==a[0]){var d=
$("*[name='"+a+"']");d[0]&&"checkbox"==d[0].type?d.prop("checked",""!=b):d.val(b)}}),"1"==d._is_html?b||(tinyMCE.execCommand("mceAddControl",!1,this.env.composebody),this.triggerEvent("aftertoggle-editor",{mode:"html"})):b&&(tinyMCE.execCommand("mceRemoveControl",!1,this.env.composebody),this.triggerEvent("aftertoggle-editor",{mode:"plain"})))};this.remove_compose_data=function(a){if(window.localStorage){var b=this.local_storage_get_item("compose.index",[]);0<=$.inArray(a,b)&&(this.local_storage_remove_item("compose."+
a),this.local_storage_set_item("compose.index",$.grep(b,function(b,e){return b!=a})))}};this.clear_compose_data=function(){if(window.localStorage){for(var a=this.local_storage_get_item("compose.index",[]),b=0;b<a.length;b++)this.local_storage_remove_item("compose."+a[b]);this.local_storage_remove_item("compose.index")}};this.change_identity=function(a,b){if(!a||!a.options)return!1;b||(b=this.env.show_sig);if(!this.env.identities_initialized&&(this.env.identities_initialized=!0,this.env.show_sig_later&&
(this.env.show_sig=!0),this.env.opened_extwin))return;var d,e,f=-1,g=a.options[a.selectedIndex].value,h=$("[name='_message']"),k=h.val(),l="1"==$("input[name='_is_html']").val(),m=this.env.identity,n=this.env.recipients_separator,p=RegExp.escape(n),u=["replyto","bcc"];for(d in u){e=u[d];var v=m&&this.env.identities[m]?this.env.identities[m][e]:"",s=g&&this.env.identities[g]?this.env.identities[g][e]:"",w=$('[name="_'+e+'"]'),t=w.val();v&&t&&(e=RegExp("\\s*"+RegExp.escape(v)+"\\s*"),t=t.replace(e,
""));e=RegExp(p+"\\s*"+p,"g");t=t.replace(e,n);e=RegExp("^[\\s"+p+"]+");t=t.replace(e,"");s&&-1==t.indexOf(s)&&-1==t.indexOf(s.replace(/"/g,""))&&(t&&(e=RegExp("["+p+"\\s]+$"),t=t.replace(e,"")+n+" "),t+=s+n+" ");(v||s)&&w.val(t).change()}this.env.signatures&&this.env.signatures[g]?(this.enable_command("insert-sig",!0),this.env.compose_commands.push("insert-sig")):this.enable_command("insert-sig",!1);l?b&&this.env.signatures&&(d=tinyMCE.get(this.env.composebody),h=d.dom.get("_rc_sig"),h||(k=d.getBody(),
m=d.getDoc(),h=m.createElement("div"),h.setAttribute("id","_rc_sig"),this.env.top_posting?(d.getWin().focus(),d=d.selection.getNode(),"BODY"==d.nodeName?(k.insertBefore(h,k.firstChild),k.insertBefore(m.createElement("br"),k.firstChild)):(k.insertBefore(h,d.nextSibling),k.insertBefore(m.createElement("br"),d.nextSibling))):(bw.ie&&k.appendChild(m.createElement("br")),k.appendChild(h))),this.env.signatures[g]&&(h.innerHTML=this.env.signatures[g].html)):(b&&m&&this.env.signatures&&this.env.signatures[m]&&
(m=this.env.signatures[m].text,m=m.replace(/\r\n/g,"\n"),f=this.env.top_posting?k.indexOf(m):k.lastIndexOf(m),0<=f&&(k=k.substring(0,f)+k.substring(f+m.length,k.length))),b&&this.env.signatures&&this.env.signatures[g]?(m=this.env.signatures[g].text,m=m.replace(/\r\n/g,"\n"),this.env.top_posting?0<=f?(k=k.substring(0,f)+m+k.substring(f,k.length),d=f-1):k?(pos=this.get_caret_pos(h.get(0)))?(k=k.substring(0,pos)+"\n"+m+"\n\n"+k.substring(pos,k.length),d=pos):(d=0,k="\n\n"+m+"\n\n"+k.replace(/^[\r\n]+/,
"")):(d=0,k="\n\n"+m):(k=k.replace(/[\r\n]+$/,""),d=!this.env.top_posting&&k.length?k.length+1:0,k+="\n\n"+m)):d=this.env.top_posting?0:k.length,h.val(k),this.set_caret_pos(h.get(0),d));this.env.identity=g;this.triggerEvent("change_identity");return!0};this.upload_file=function(a,b){if(!a)return!1;var d=0,e=0;$("input[type=file]",a).each(function(a,b){var f=b.files?b.files.length:b.value?1:0;if(b.files)for(a=0;a<f;a++)d+=b.files[a].size;e+=f});if(e){if(this.env.max_filesize&&this.env.filesizeerror&&
d>this.env.max_filesize){this.display_message(this.env.filesizeerror,"error");return}var f=this.async_upload_form(a,b||"upload",function(a){var b,d="";try{this.contentDocument?b=this.contentDocument:this.contentWindow&&(b=this.contentWindow.document),d=b.childNodes[0].innerHTML}catch(e){}d.match(/add2attachment/)||bw.opera&&(!rcmail.env.uploadframe||rcmail.env.uploadframe!=a.data.ts)||(d.match(/display_message/)||rcmail.display_message(rcmail.get_label("fileuploaderror"),"error"),rcmail.remove_from_attachment_list(a.data.ts));
bw.opera&&(rcmail.env.uploadframe=a.data.ts)}),g="<span>"+this.get_label("uploading"+(1<e?"many":""))+"</span>",h=f.replace(/^rcmupload/,"");this.add2attachment_list(h,{name:"",html:g,classname:"uploading",frame:f,complete:!1});this.env.upload_progress_time&&this.upload_progress_start("upload",h)}this.gui_objects.attachmentform=a;return!0};this.add2attachment_list=function(a,b,d){if(!this.gui_objects.attachmentlist)return!1;!b.complete&&l.env.loadingicon&&(b.html='<img src="'+l.env.loadingicon+'" alt="" class="uploading" />'+
b.html);!b.complete&&b.frame&&(b.html='<a title="'+this.get_label("cancel")+'" onclick="return rcmail.cancel_attachment_upload(\''+a+"', '"+b.frame+'\');" href="#cancelupload" class="cancelupload">'+(this.env.cancelicon?'<img src="'+this.env.cancelicon+'" alt="" />':this.get_label("cancel"))+"</a>"+b.html);var e,f=$("<li>");f.attr("id",a).addClass(b.classname).html(b.html).on("mouseover",function(){rcube_webmail.long_subject_title_ex(this,0)});d&&(e=document.getElementById(d))?f.replaceAll(e):f.appendTo(this.gui_objects.attachmentlist);
d&&this.env.attachments[d]&&delete this.env.attachments[d];this.env.attachments[a]=b;return!0};this.remove_from_attachment_list=function(a){delete this.env.attachments[a];$("#"+a).remove()};this.remove_attachment=function(a){a&&this.env.attachments[a]&&this.http_post("remove-attachment",{_id:this.env.compose_id,_file:a});return!0};this.cancel_attachment_upload=function(a,b){if(!a||!b)return!1;this.remove_from_attachment_list(a);$("iframe[name='"+b+"']").remove();return!1};this.upload_progress_start=
function(a,b){setTimeout(function(){rcmail.http_request(a,{_progress:b})},1E3*this.env.upload_progress_time)};this.upload_progress_update=function(a){var b=$("#"+a.name+"> span");b.length&&a.text&&(b.text(a.text),a.done||this.upload_progress_start(a.action,a.name))};this.add_contact=function(a){a&&this.http_post("addcontact",{_address:a});return!0};this.qsearch=function(a){if(""!=a){var b=this.set_busy(!0,"searching");a=this.search_params(a);this.message_list?this.clear_message_list():this.contact_list&&
...
this.ksearch_selected&&void 0!==this.ksearch_selected&&this.ksearch_value};this.ksearch_select=function(a){var b=$("#rcmksearchSelected");b[0]&&a&&b.removeAttr("id").removeClass("selected");a&&($(a).attr("id","rcmksearchSelected").addClass("selected"),this.ksearch_selected=a._rcm_id)};this.insert_recipient=function(a){if(null!==a&&this.env.contacts[a]&&this.ksearch_input){var b=this.ksearch_input.value,d=this.get_caret_pos(this.ksearch_input),d=b.lastIndexOf(this.ksearch_value,d),e=!1,f="",g=b.substring(0,
d),b=b.substring(d+this.ksearch_value.length,b.length);this.ksearch_destroy();"object"===typeof this.env.contacts[a]&&this.env.contacts[a].id?(f+=this.env.contacts[a].name+this.env.recipients_delimiter,this.group2expand[this.env.contacts[a].id]=$.extend({input:this.ksearch_input},this.env.contacts[a]),this.http_request("mail/group-expand",{_source:this.env.contacts[a].source,_gid:this.env.contacts[a].id},!1)):"string"===typeof this.env.contacts[a]&&(f=this.env.contacts[a]+this.env.recipients_delimiter,
...
null,this.compose_type_activity++)};this.ksearch_get_results=function(a){var b=this.ksearch_input?this.ksearch_input.value:null;if(null!==b){this.ksearch_pane&&this.ksearch_pane.is(":visible")&&this.ksearch_pane.hide();var d=this.get_caret_pos(this.ksearch_input),e=b.lastIndexOf(this.env.recipients_separator,d-1),b=b.substring(e+1,d),e=this.env.autocomplete_min_length,d=this.ksearch_data,b=$.trim(b);if(b!=this.ksearch_value)if(this.ksearch_destroy(),b.length&&b.length<e)this.ksearch_info||(this.ksearch_info=
this.display_message(this.get_label("autocompletechars").replace("$min",e)));else if(e=this.ksearch_value,this.ksearch_value=b,b.length&&(!(e&&e.length&&b.startsWith(e)&&(!d||0>=d.num)&&this.env.contacts)||this.env.contacts.length)){var f,g,e=(new Date).getTime(),b={_search:b,_id:e},d=a&&a.threads?a.threads:1;f=a&&a.sources?a.sources:[];a=a&&a.action?a.action:"mail/autocomplete";this.ksearch_data={id:e,sources:f.slice(),action:a,locks:[],requests:[],num:f.length};for(e=0;e<d;e++){f=this.ksearch_data.sources.shift();
if(1<d&&void 0===f)break;b._source=f?f:"";f=this.display_message(this.get_label("searching"),"loading");g=this.http_post(a,b,f);this.ksearch_data.locks.push(f);this.ksearch_data.requests.push(g)}}}};this.ksearch_query_results=function(a,b,d){if(this.ksearch_value&&(!this.ksearch_input||b==this.ksearch_value)){var e,f,g,h,k,q=this.ksearch_value;b=this.ksearch_data;var m=this.env.autocomplete_max?this.env.autocomplete_max:15;this.ksearch_pane||(g=$("<ul>"),this.ksearch_pane=$("<div>").attr("id","rcmKSearchpane").css({position:"absolute",
"z-index":3E4}).append(g).appendTo(document.body),this.ksearch_pane.__ul=g[0]);g=this.ksearch_pane.__ul;d&&this.ksearch_pane.data("reqid")==d?m-=g.childNodes.length:(this.ksearch_pane.data("reqid",d),g.innerHTML="",this.env.contacts=[],e=$(this.ksearch_input).offset(),this.ksearch_pane.css({left:e.left+"px",top:e.top+this.ksearch_input.offsetHeight+"px",display:"none"}));if(a&&(f=a.length))for(e=0;e<f&&0<m;e++)k="object"===typeof a[e]?a[e].name:a[e],h=document.createElement("LI"),h.innerHTML=k.replace(RegExp("("+
RegExp.escape(q)+")","ig"),"##$1%%").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/##([^%]+)%%/g,"<b>$1</b>"),h.onmouseover=function(){l.ksearch_select(this)},h.onmouseup=function(){l.ksearch_click(this)},h._rcm_id=this.env.contacts.length+e,g.appendChild(h),m-=1;g.childNodes.length&&(this.ksearch_pane.show(),this.env.contacts.length||($("li:first",g).attr("id","rcmksearchSelected").addClass("selected"),this.ksearch_selected=0));f&&(this.env.contacts=this.env.contacts.concat(a));if(b.id==d)if(b.num--,
0<m&&b.sources.length){if(a=b.sources.shift())a={_search:q,_id:d,_source:a},d=this.display_message(this.get_label("searching"),"loading"),b=this.http_post(b.action,a,d),this.ksearch_data.locks.push(d),this.ksearch_data.requests.push(b)}else m||(this.ksearch_msg||(this.ksearch_msg=this.display_message(this.get_label("autocompletemore"))),this.ksearch_abort())}};this.ksearch_click=function(a){this.ksearch_input&&this.ksearch_input.focus();this.insert_recipient(a._rcm_id);this.ksearch_hide()};this.ksearch_blur=
function(){this.ksearch_timer&&clearTimeout(this.ksearch_timer);this.ksearch_input=null;this.ksearch_hide()};this.ksearch_hide=function(){this.ksearch_selected=null;this.ksearch_value="";this.ksearch_pane&&this.ksearch_pane.hide();this.ksearch_destroy()};this.ksearch_destroy=function(){this.ksearch_abort();this.ksearch_info&&this.hide_message(this.ksearch_info);this.ksearch_msg&&this.hide_message(this.ksearch_msg);this.ksearch_msg=this.ksearch_info=this.ksearch_data=null};this.ksearch_abort=function(){var a,
b,d=this.ksearch_data;if(d)for(a=0,b=d.locks.length;a<b;a++)this.abort_request({request:d.requests[a],lock:d.locks[a]})};this.contactlist_keypress=function(a){a.key_pressed==a.DELETE_KEY&&this.command("delete")};this.contactlist_select=function(a){this.preview_timer&&clearTimeout(this.preview_timer);var b,d,e,f,g=this,h=!1,k=this.env.source?this.env.address_sources[this.env.source]:null;(d=a.get_single_selection())?this.preview_timer=setTimeout(function(){g.load_contact(d,"show")},200):this.env.contentframe&&
...
this.env.group&&0<a.selection.length&&h);this.enable_command("compose",this.env.group||0<a.selection.length);this.enable_command("export-selected","copy",0<a.selection.length);this.enable_command("edit",d&&h);this.enable_command("delete","move",0<a.selection.length&&h);return!1};this.list_contacts=function(a,b,d){var e,f={},g=window;a||(a=this.env.source);if(d&&this.current_page==d&&a==this.env.source&&b==this.env.group)return!1;a!=this.env.source?(d=this.env.current_page=1,this.reset_qsearch()):
b!=this.env.group&&(d=this.env.current_page=1);this.env.search_id?e="S"+this.env.search_id:this.env.search_request||(e=b?"G"+a+b:a);this.env.source=a;this.env.group=b;var h=$.inArray(this.env.group,this.env.address_group_stack);this.env.address_group_stack=0>h?[]:this.env.address_group_stack.slice(0,h);this.env.group?(this.env.address_group_stack.push(this.env.group),e="G"+a+this.env.address_group_stack[0]):this.gui_objects.addresslist_title&&$(this.gui_objects.addresslist_title).html(this.get_label("contacts"));
this.select_folder(e,"",!0);if(this.gui_objects.contactslist)this.list_contacts_remote(a,b,d);else{if(e=this.get_frame_window(this.env.contentframe))g=e,f._framed=1;b&&(f._gid=b);d&&(f._page=d);a&&(f._source=a);this.env.search_request&&(f._search=this.env.search_request);this.set_busy(!0,"loading");this.location_href(f,g)}};this.list_contacts_remote=function(a,b,d){this.list_contacts_clear();var e={},f=this.set_busy(!0,"loading");a&&(e._source=a);d&&(e._page=d);b&&(e._gid=b);this.env.source=a;this.env.group=
...
($('<a href="#list">...</a>').addClass("poplink").appendTo(b).click(function(a){return l.command("popgroup","",this)}),b.append("&nbsp;&raquo;&nbsp;"));b.append($("<span>").text(a.name))}this.triggerEvent("groupupdate",a)};this.load_contact=function(a,b,d){var e,f={},g=window,h=this.contact_list?this.contact_list.data[a]:null;if(e=this.get_frame_window(this.env.contentframe))f._framed=1,g=e,this.show_contentframe(!0),a||this.contact_list.clear_selection(),this.enable_command("compose",h&&h.email),
this.enable_command("export-selected",h&&"group"!=h._type);else if(d)return!1;!b||!a&&"add"!=b||this.drag_active||(this.env.group&&(f._gid=this.env.group),f._action=b,f._source=this.env.source,f._cid=a,this.location_href(f,g,!0));return!0};this.group_member_change=function(a,b,d,e){a="add"==a?"add":"del";var f=this.get_label("add"==a?"addingmember":"removingmember"),f=this.display_message(f,"loading");this.http_post("group-"+a+"members",{_cid:b,_source:d,_gid:e},f)};this.contacts_drag_menu=function(a,
b){var d="group"==b.type?b.source:b.id,e=this.env.source;if(!this.env.address_sources[d]||this.env.address_sources[d].readonly)return!0;""==e&&1==this.env.selection_sources.length&&(e=this.env.selection_sources[0]);return"group"==b.type&&d==e?(e=this.contact_list.get_selection().join(","),this.group_member_change("add",e,d,b.id),!0):this.commands.move||rcube_event.get_modifier(a)==SHIFT_KEY?this.drag_menu(a,b):(this.copy_contacts(b),!0)};this.copy_contacts=function(a){var b="group"==a.type?a.source:
a.id,d=this.env.source,e=this.env.group?this.env.group:"",f=this.contact_list.get_selection().join(",");f&&this.env.address_sources[b]&&!this.env.address_sources[b].readonly&&(""==d&&1==this.env.selection_sources.length&&(d=this.env.selection_sources[0]),"group"==a.type?b!=d&&(d=this.display_message(this.get_label("copyingcontact"),"loading"),a={_cid:f,_source:this.env.source,_to:b,_togid:a.id,_gid:e},this.http_post("copy",a,d)):a.id!=d&&(d=this.display_message(this.get_label("copyingcontact"),"loading"),
a={_cid:f,_source:this.env.source,_to:a.id,_gid:e},this.http_post("copy",a,d)))};this.move_contacts=function(a){var b="group"==a.type?a.source:a.id,d=this.env.source;this.env.address_sources[b]&&!this.env.address_sources[b].readonly&&(""==d&&1==this.env.selection_sources.length&&(d=this.env.selection_sources[0]),"group"==a.type?b!=d&&this._with_selected_contacts("move",{_to:b,_togid:a.id}):a.id!=d&&this._with_selected_contacts("move",{_to:a.id}))};this.delete_contacts=function(){if(this.env.source&&
this.env.address_sources[this.env.source].undelete||confirm(this.get_label("deletecontactconfirm")))return this._with_selected_contacts("delete")};this._with_selected_contacts=function(a,b){var d=this.contact_list?this.contact_list.get_selection():[];if(d.length||this.env.cid){var e,f=[],g=this.display_message(this.get_label("delete"==a?"contactdeleting":"movingcontact"),"loading");if(this.env.cid)f.push(this.env.cid);else{for(e=0;e<d.length;e++)id=d[e],f.push(id),this.contact_list.remove_row(id,
e==d.length-1);1==d.length&&this.show_contentframe(!1)}b||(b={});b._source=this.env.source;b._from=this.env.action;b._cid=f.join(",");this.env.group&&(b._gid=this.env.group);this.env.search_request&&(b._search=this.env.search_request);this.http_post(a,b,g);return!0}};this.update_contact_row=function(a,b,d,e,f){var g=this.contact_list;a=this.html_identifier(a);g.rows[a]||(a=a+"-"+e,d&&(d=d+"-"+e));g.update_row(a,b,d,!0);g.data[a]=f};this.add_contact_row=function(a,b,d,e){if(!this.gui_objects.contactslist)return!1;
var f,g=this.contact_list,h={cols:[]};h.id="rcmrow"+this.html_identifier(a);h.className="contact "+(d||"");g.in_selection(a)&&(h.className+=" selected");for(f in b)d={},d.className=String(f).toLowerCase(),d.innerHTML=b[f],h.cols.push(d);g.data[a]=e;g.insert_row(h);this.enable_command("export",0<g.rowcount)};this.init_contact_form=function(){var a=this,b;if(this.env.coltypes)for(b in this.set_photo_actions($("#ff_photo").val()),this.env.coltypes)this.init_edit_field(b,null);$(".contactfieldgroup .row a.deletebutton").click(function(){a.delete_edit_field(this);
return!1});$("select.addfieldmenu").change(function(b){a.insert_edit_field($(this).val(),$(this).attr("rel"),this);this.selectedIndex=0});$.datepicker&&this.env.date_format&&($.datepicker.setDefaults({dateFormat:this.env.date_format,changeMonth:!0,changeYear:!0,yearRange:"-100:+10",showOtherMonths:!0,selectOtherMonths:!0,onSelect:function(a){$(this).focus().val(a)}}),$("input.datepicker").datepicker());"search"==this.env.action&&$(this.gui_objects.editform).append($('<input type="submit">').hide()).submit(function(){$("input.mainaction").click();
return!1})};this.group_create=function(){this.add_input_row("contactgroup")};this.group_rename=function(){if(this.env.group&&this.gui_objects.folderlist){if(!this.name_input){this.enable_command("list","listgroup",!1);this.name_input=$("<input>").attr("type","text").val(this.env.contactgroups["G"+this.env.source+this.env.group].name);this.name_input.bind("keydown",function(a){return rcmail.add_input_keydown(a)});this.env.group_renaming=!0;var a,b=this.get_folder_li("G"+this.env.source+this.env.group,
...
this.list_contacts(a.source,0)};this.add_input_row=function(a){if(this.gui_objects.folderlist){if(!this.name_input){this.name_input=$("<input>").attr("type","text").data("tt",a);this.name_input.bind("keydown",function(a){return rcmail.add_input_keydown(a)});this.name_input_li=$("<li>").addClass(a).append(this.name_input);var b;a="contactsearch"==a?this.gui_objects.folderlist:$("ul.groups",this.get_folder_li(this.env.source,"",!0));b=$("li:last",a);b.length?this.name_input_li.insertAfter(b):(this.name_input_li.appendTo(a),
a.show())}this.name_input.select().focus()}};this.group_remove_selected=function(){l.http_post("group-delmembers",{_cid:this.contact_list.selection,_source:this.env.source,_gid:this.env.group})};this.remove_group_contacts=function(a){if("undefined"!=typeof this.env.group&&this.env.group===a.gid){var b=this.contact_list.get_selection();for(a=0;a<b.length;a++)id=b[a],this.contact_list.remove_row(id,a==b.length-1)}};this.add_input_keydown=function(a){var b=rcube_event.get_keycode(a),d=$(a.target);a=
d.data("tt");if(13==b){if(b=d.val())d=this.set_busy(!0,"loading"),"contactsearch"==a?this.http_post("search-create",{_search:this.env.search_request,_name:b},d):this.env.group_renaming?this.http_post("group-rename",{_source:this.env.source,_gid:this.env.group,_name:b},d):this.http_post("group-create",{_source:this.env.source,_name:b},d);return!1}27==b&&this.reset_add_input();return!0};this.reset_add_input=function(){if(this.name_input){var a=this.name_input.parent();this.env.group_renaming?(a.children().last().show(),
this.env.group_renaming=!1):1==$("li",a.parent()).length&&a.parent().hide();this.name_input.remove();this.name_input_li&&this.name_input_li.remove();this.name_input=this.name_input_li=null}this.enable_command("list","listgroup",!0)};this.insert_contact_group=function(a){this.reset_add_input();a.type="group";var b="G"+a.source+a.id,d=$("<a>").attr("href","#").attr("rel",a.source+":"+a.id).click(function(){return rcmail.command("listgroup",a,this)}).html(a.name);this.env.contactfolders[b]=this.env.contactgroups[b]=
a;this.treelist.insert({id:b,html:d,classes:["contactgroup"]},a.source,!0);this.triggerEvent("group_insert",{id:a.id,source:a.source,name:a.name,li:this.treelist.get_item(b)})};this.update_contact_group=function(a){this.reset_add_input();var b="G"+a.source+a.id,d={};if(a.newid){var e="G"+a.source+a.newid,f=$.extend({},a);this.env.contactfolders[e]=this.env.contactfolders[b];this.env.contactfolders[e].id=a.newid;this.env.group=a.newid;delete this.env.contactfolders[b];delete this.env.contactgroups[b];
f.id=a.newid;f.type="group";d.id=e;d.html=$("<a>").attr("href","#").attr("rel",a.source+":"+a.newid).click(function(){return rcmail.command("listgroup",f,this)}).html(a.name)}else $(this.treelist.get_item(b)).children().first().html(a.name),this.env.contactfolders[b].name=this.env.contactgroups[b].name=a.name;this.treelist.update(b,d,!0);this.triggerEvent("group_update",{id:a.id,source:a.source,name:a.name,li:this.treelist.get_item(b),newid:a.newid})};this.update_group_commands=function(){var a=""!=
this.env.source?this.env.address_sources[this.env.source]:null;this.enable_command("group-create",a&&a.groups&&!a.readonly);this.enable_command("group-rename","group-delete",a&&a.groups&&this.env.group&&!a.readonly)};this.init_edit_field=function(a,b){var d=this.env.coltypes[a].label;b||(b=$(".ff_"+a));d&&b.placeholder(d)};this.insert_edit_field=function(a,b,d){var e=$("#ff_"+a);if(e.length)e.show().focus(),$(d).children('option[value="'+a+'"]').prop("disabled",!0);else{$(".ff_"+a);e=$("#contactsection"+
b+" .contactcontroller"+a);if(!e.length){b=$("#contactsection"+b);var f=$(".contactfieldgroup",b).last(),e=$("<fieldset>").addClass("contactfieldgroup contactcontroller"+a);f.length?e.insertAfter(f):b.prepend(e)}if(e.length&&"FIELDSET"==e.get(0).nodeName){var g;b=this.env.coltypes[a];var f=$("<div>").addClass("row"),h=$("<div>").addClass("contactfieldcontent data"),k=$("<div>").addClass("contactfieldlabel label");b.subtypes_select?k.html(b.subtypes_select):k.html(b.label);var q=1!=b.limit?"[]":"";
if("text"==b.type||"date"==b.type)g=$("<input>").addClass("ff_"+a).attr({type:"text",name:"_"+a+q,size:b.size}).appendTo(h),this.init_edit_field(a,g),"date"==b.type&&$.datepicker&&g.datepicker();else if("textarea"==b.type)g=$("<textarea>").addClass("ff_"+a).attr({name:"_"+a+q,cols:b.size,rows:b.rows}).appendTo(h),this.init_edit_field(a,g);else if("composite"==b.type){var m,n,p=[],u=[];if(g=this.env[a+"_template"])for(m=0;m<g.length;m++)p.push(g[m][1]),u.push(g[m][2]);else for(m in b.childs)p.push(m);
for(var v=0;v<p.length;v++)m=p[v],g=b.childs[m],g=$("<input>").addClass("ff_"+m).attr({type:"text",name:"_"+m+q,size:g.size}).appendTo(h),h.append(u[v]||" "),this.init_edit_field(m,g),n||(n=g);g=n}else if("select"==b.type){g=$("<select>").addClass("ff_"+a).attr("name","_"+a+q).appendTo(h);var s=g.attr("options");s[s.length]=new Option("---","");b.options&&$.each(b.options,function(a,b){s[s.length]=new Option(b,a)})}g&&($('<a href="#del"></a>').addClass("contactfieldbutton deletebutton").attr({title:this.get_label("delete"),
rel:a}).html(this.env.delbutton).click(function(){l.delete_edit_field(this);return!1}).appendTo(h),f.append(k).append(h).appendTo(e.show()),g.first().focus(),b.count||(b.count=0),++b.count==b.limit&&b.limit&&$(d).children('option[value="'+a+'"]').prop("disabled",!0))}}};this.delete_edit_field=function(a){var b=$(a).attr("rel"),d=this.env.coltypes[b],e=$(a).parents("fieldset.contactfieldgroup"),f=e.parent().find("select.addfieldmenu");0>=--d.count&&d.visible?$(a).parent().children("input").val("").blur():
($(a).parents("div.row").remove(),e.children("div.row").length||e.hide());f.length&&(a=f.children('option[value="'+b+'"]'),a.length?a.prop("disabled",!1):$("<option>").attr("value",b).html(d.label).appendTo(f),f.show())};this.upload_contact_photo=function(a){a&&a.elements._photo.value&&(this.async_upload_form(a,"upload-photo",function(a){rcmail.set_busy(!1,null,rcmail.file_upload_id)}),this.file_upload_id=this.set_busy(!0,"uploading"))};this.replace_contact_photo=function(a){var b="-del-"==a?this.env.photo_placeholder:
...
this.env.coltypes.photo?!0:!1);this.enable_command("delete-photo",this.env.coltypes.photo&&"-del-"!=a)};this.advanced_search=function(){var a,b={_form:1,_action:"search"},d=window;if(a=this.get_frame_window(this.env.contentframe))b._framed=1,d=a,this.contact_list.clear_selection();this.location_href(b,d,!0);return!0};this.unselect_directory=function(){this.select_folder("");this.enable_command("search-delete",!1)};this.insert_saved_search=function(a,b){this.reset_add_input();var d="S"+b,e=$("<a>").attr("href",
"#").attr("rel",b).click(function(){return rcmail.command("listsearch",b,this)}).html(a),f={name:a,id:b};this.treelist.insert({id:d,html:e,classes:["contactsearch"]},null,"contactsearch");this.select_folder(d,"",!0);this.enable_command("search-delete",!0);this.env.search_id=b;this.triggerEvent("abook_search_insert",f)};this.search_create=function(){this.add_input_row("contactsearch")};this.search_delete=function(){if(this.env.search_request){var a=this.set_busy(!0,"savedsearchdeleting");this.http_post("search-delete",
{_sid:this.env.search_id},a)}};this.remove_search_item=function(a){this.treelist.remove("S"+a)&&this.triggerEvent("search_delete",{id:a,li:void 0});this.env.search_id=null;this.env.search_request=null;this.list_contacts_clear();this.reset_qsearch();this.enable_command("search-delete","search-create",!1)};this.listsearch=function(a){var b=this.set_busy(!0,"searching");this.contact_list&&this.list_contacts_clear();this.reset_qsearch();this.select_folder("S"+a,"",!0);this.env.current_page=1;this.http_request("search",
{_sid:a},b)};this.section_select=function(a){var b;b=a.get_single_selection();a=window;var d={_action:"edit-prefs",_section:b};if(b){if(b=this.get_frame_window(this.env.contentframe))d._framed=1,a=b;this.location_href(d,a,!0)}return!0};this.identity_select=function(a){var b;if(b=a.get_single_selection())this.enable_command("delete",1<a.rowcount&&2>this.env.identities_level),this.load_identity(b,"edit-identity")};this.load_identity=function(a,b){if("edit-identity"==b&&(!a||a==this.env.iid))return!1;
var d,e=window,f={_action:b,_iid:a};if(d=this.get_frame_window(this.env.contentframe))f._framed=1,e=d;(a||"add-identity"==b)&&this.location_href(f,e,!0);return!0};this.delete_identity=function(a){var b=this.identity_list.get_selection();if(b.length||this.env.iid)return a||(a=this.env.iid?this.env.iid:b[0]),confirm(this.get_label("deleteidentityconfirm"))&&this.goto_url("delete-identity",{_iid:a,_token:this.env.request_token},!0),!0};this.update_identity_row=function(a,b,d){var e=this.identity_list;
a=this.html_identifier(a);d?(e.insert_row({id:"rcmrow"+a,cols:[{className:"mail",innerHTML:b}]}),e.select(a)):e.update_row(a,[b])};this.update_response_row=function(a,b){var d=this.responses_list;d&&b?d.update_row(b,[a.name],a.key,!0):d&&(d.insert_row({id:"rcmrow"+a.key,cols:[{className:"name",innerHTML:a.name}]}),d.select(a.key))};this.remove_response=function(a){var b;this.env.textresponses&&delete this.env.textresponses[a];this.responses_list&&(this.responses_list.remove_row(a),this.env.contentframe&&
(b=this.get_frame_window(this.env.contentframe))&&(b.location.href=this.env.blankpage))};this.init_subscription_list=function(){var a=this,b=RegExp.escape(this.env.delimiter);this.last_sub_rx=RegExp("["+b+"]?[^"+b+"]+$");this.subscription_list=new rcube_list_widget(this.gui_objects.subscriptionlist,{multiselect:!1,draggable:!0,keyboard:!1,toggleselect:!0});this.subscription_list.addEventListener("select",function(b){a.subscription_select(b)}).addEventListener("dragstart",function(b){a.drag_active=
!0}).addEventListener("dragend",function(b){a.subscription_move_folder(b)}).addEventListener("initrow",function(b){b.obj.onmouseover=function(){a.focus_subscription(b.id)};b.obj.onmouseout=function(){a.unfocus_subscription(b.id)}}).init();$("#mailboxroot").mouseover(function(){a.focus_subscription(this.id)}).mouseout(function(){a.unfocus_subscription(this.id)})};this.focus_subscription=function(a){var b,d;this.drag_active&&this.env.mailbox&&(b=document.getElementById(a))&&this.env.subscriptionrows[a]&&
null!==(d=this.env.subscriptionrows[a][0])&&this.check_droptarget(d)&&!this.env.subscriptionrows[this.get_folder_row_id(this.env.mailbox)][2]&&d!=this.env.mailbox.replace(this.last_sub_rx,"")&&!d.startsWith(this.env.mailbox+this.env.delimiter)&&(this.env.dstfolder=d,$(b).addClass("droptarget"))};this.unfocus_subscription=function(a){var b=$("#"+a);this.env.dstfolder=null;this.env.subscriptionrows[a]&&b.length?b.removeClass("droptarget"):$(this.subscription_list.frame).removeClass("droptarget")};this.subscription_select=
function(a){var b,d;a&&(b=a.get_single_selection())&&(d=this.env.subscriptionrows["rcmrow"+b])?(this.env.mailbox=d[0],this.show_folder(d[0]),this.enable_command("delete-folder",!d[2])):(this.env.mailbox=null,this.show_contentframe(!1),this.enable_command("delete-folder","purge",!1))};this.subscription_move_folder=function(a){this.env.mailbox&&null!==this.env.dstfolder&&this.env.dstfolder!=this.env.mailbox&&this.env.dstfolder!=this.env.mailbox.replace(this.last_sub_rx,"")&&(a=this.env.mailbox.split(this.env.delimiter).pop(),
a=""===this.env.dstfolder?a:this.env.dstfolder+this.env.delimiter+a,a!=this.env.mailbox&&(this.http_post("rename-folder",{_folder_oldname:this.env.mailbox,_folder_newname:a},this.set_busy(!0,"foldermoving")),this.subscription_list.draglayer.hide()));this.drag_active=!1;this.unfocus_subscription(this.get_folder_row_id(this.env.dstfolder))};this.create_folder=function(){this.show_folder("",this.env.mailbox)};this.delete_folder=function(a){a=this.get_folder_row_id(a?a:this.env.mailbox);if((a=this.env.subscriptionrows[a][0])&&
confirm(this.get_label("deletefolderconfirm"))){var b=this.set_busy(!0,"folderdeleting");this.http_post("delete-folder",{_mbox:a},b)}};this.add_folder_row=function(a,b,d,e,f,g){if(!this.gui_objects.subscriptionlist)return!1;var h,k,l,m,n=[],p=[],u=[],v=this.gui_objects.subscriptionlist.tBodies[0];h=$("tr",v).get(1);var s="rcmrow"+(new Date).getTime();if(!h)return this.goto_url("folders"),!1;h=$(h).clone(!0);h.attr({id:s,"class":g});h.find("td:first").html(b);$('input[name="_subscribed[]"]',h).val(a).prop({checked:e?
!0:!1,disabled:d?!0:!1});this.env.subscriptionrows[s]=[a,b,!1];var w=RegExp(RegExp.escape(this.env.delimiter),"g"),t=String.fromCharCode(0);$.each(this.env.subscriptionrows,function(a,b){if(4>b.length){var d=b[0],d=d.replace(w,t);b.push(d)}n.push(b)});n.sort(function(a,b){var d=a.length-1;n1=a[d];n2=b[d];return n1<n2?-1:1});for(k in n)n[k][2]?(b=n[k][0]+this.env.delimiter,b!=this.env.prefix_ns&&(u.push(n[k][0]),l=b)):l&&n[k][0].startsWith(l)?u.push(n[k][0]):(p.push(n[k][0]),l=null);for(k=0;k<u.length;k++)a.startsWith(u[k]+
this.env.delimiter)&&(m=this.get_folder_row_id(u[k]));for(k=0;!m&&k<p.length;k++)k&&p[k]==a&&(m=this.get_folder_row_id(p[k-1]));m?$("#"+m).after(h):h.appendTo(v);this.subscription_list.clear_selection();f||this.init_subscription_list();h=h.get(0);h.scrollIntoView&&h.scrollIntoView();return h};this.replace_folder_row=function(a,b,d,e,f){if(!this.gui_objects.subscriptionlist)return this.is_framed?parent.rcmail.replace_folder_row(a,b,d,e,f):!1;var g,h,k,l,m=this.get_folder_row_id(a),n=a.length;g=$('input[name="_subscribed[]"]',
$("#"+m)).prop("checked");var p=this.get_subfolders(a);if(a==b)$("#"+m).attr("class",f||""),this.subscription_list.focus();else{this._remove_folder_row(m);e=$(this.add_folder_row(b,d,e,g,!0,f));if(d=p.length)l=a.split(this.env.delimiter).length-b.split(this.env.delimiter).length;for(a=0;a<d;a++)if(m=p[a],g=this.env.subscriptionrows[m][0],f=this.env.subscriptionrows[m][1],h=$("#"+m),k=h.clone(!0),h.remove(),e.after(k),e=k,g=b+g.slice(n),$('input[name="_subscribed[]"]',e).val(g),this.env.subscriptionrows[m][0]=
g,0!=l){if(0<l)for(g=l;0<g;g--)f=f.replace(/^&nbsp;&nbsp;&nbsp;&nbsp;/,"");else for(g=l;0>g;g++)f="&nbsp;&nbsp;&nbsp;&nbsp;"+f;e.find("td:first").html(f);this.env.subscriptionrows[m][1]=f}this.init_subscription_list()}};this.remove_folder_row=function(a,b){var d,e,f=[];d=this.get_folder_row_id(a);b&&(f=this.get_subfolders(a));this._remove_folder_row(d);d=0;for(e=f.length;d<e;d++)this._remove_folder_row(f[d])};this._remove_folder_row=function(a){this.subscription_list.remove_row(a.replace(/^rcmrow/,
...
function(a){(a=this.get_folder_row_id(a))&&$('input[name="_subscribed[]"]',$("#"+a)).prop("disabled",!0)};this.folder_size=function(a){var b=this.set_busy(!0,"loading");this.http_post("folder-size",{_mbox:a},b)};this.folder_size_update=function(a){$("#folder-size").replaceWith(a)};var x=function(a,b){var d=document.getElementById(b.id);if(d){var e=!1;"image"==b.type&&(d=d.parentNode,e=!0);d._command=a;d._id=b.id;b.sel&&(d.onmousedown=function(a){return rcmail.button_sel(this._command,this._id)},d.onmouseup=
function(a){return rcmail.button_out(this._command,this._id)},e&&((new Image).src=b.sel));b.over&&(d.onmouseover=function(a){return rcmail.button_over(this._command,this._id)},d.onmouseout=function(a){return rcmail.button_out(this._command,this._id)},e&&((new Image).src=b.over))}};this.init_buttons=function(){for(var a in this.buttons)if("string"===typeof a)for(var b=0;b<this.buttons[a].length;b++)x(a,this.buttons[a][b]);this.set_button(this.task,"sel")};this.set_button=function(a,b){var d,e,f,g=
this.buttons[a],h=g?g.length:0;for(d=0;d<h;d++)if(e=g[d],f=document.getElementById(e.id))"image"!=e.type||e.status?e.status||(e.pas=String(f.className)):(e.pas=f._original_src?f._original_src:f.src,f.runtimeStyle&&f.runtimeStyle.filter&&f.runtimeStyle.filter.match(/src=['"]([^'"]+)['"]/)&&(e.pas=RegExp.$1)),"image"==e.type&&e[b]?(e.status=b,f.src=e[b]):void 0!==e[b]&&(e.status=b,f.className=e[b]),"input"==e.type&&(e.status=b,f.disabled=!b)};this.set_alttext=function(a,b){var d,e,f,g,h=this.buttons[a],
k=h?h.length:0;for(d=0;d<k;d++)e=h[d],f=document.getElementById(e.id),"image"==e.type&&f?(f.setAttribute("alt",this.get_label(b)),(g=f.parentNode)&&"a"==g.tagName.toLowerCase()&&g.setAttribute("title",this.get_label(b))):f&&f.setAttribute("title",this.get_label(b))};this.button_over=function(a,b){this.button_event(a,b,"over")};this.button_sel=function(a,b){this.button_event(a,b,"sel")};this.button_out=function(a,b){this.button_event(a,b,"act")};this.button_event=function(a,b,d){var e,f,g,h=this.buttons[a],
k=h?h.length:0;for(e=0;e<k;e++)f=h[e],f.id==b&&"act"==f.status&&(f[d]&&(g=document.getElementById(f.id))&&(g["image"==f.type?"src":"className"]=f[d]),"sel"==d&&(this.buttons_sel[b]=a))};this.set_pagetitle=function(a){a&&document.title&&(document.title=a)};this.display_message=function(a,b,d){if(this.is_framed())return parent.rcmail.display_message(a,b,d);if(!this.gui_objects.message)return"loading"!=b&&(this.pending_message=[a,b,d]),1;b=b?b:"notice";var e=this,f=this.html_identifier(a),g=b+(new Date).getTime();
d||(d=this.message_time*("error"==b||"warning"==b?2:1));"loading"==b&&(f="loading",d=1E3*this.env.request_timeout,a||(a=this.get_label("loading")));if(this.messages[f])return this.messages[f].obj&&this.messages[f].obj.html(a),"loading"==b&&this.messages[f].labels.push({id:g,msg:a}),this.messages[f].elements.push(g),setTimeout(function(){e.hide_message(g,"loading"==b)},d),g;var h=$("<div>").addClass(b).html(a).data("key",f);$(this.gui_objects.message).append(h).show();this.messages[f]={obj:h,elements:[g]};
"loading"==b?this.messages[f].labels=[{id:g,msg:a}]:h.click(function(){return e.hide_message(h)});this.triggerEvent("message",{message:a,type:b,timeout:d,object:h});0<d&&setTimeout(function(){e.hide_message(g,"loading"!=b)},d);return g};this.hide_message=function(a,b){if(this.is_framed())return parent.rcmail.hide_message(a,b);if(this.gui_objects.message){var d,e,f,g,h=this.messages;if("object"===typeof a)g=$(a),d=g.data("key"),this.hide_message_object(g,b),h[d]&&delete h[d];else for(d in h)for(e in h[d].elements)if(h[d]&&
h[d].elements[e]==a)if(h[d].elements.splice(e,1),!h[d].elements.length)this.hide_message_object(h[d].obj,b),delete h[d];else if("loading"==d)for(f in h[d].labels)h[d].labels[f].id==a?delete h[d].labels[f]:(g=h[d].labels[f].msg,h[d].obj.html(g))}};this.hide_message_object=function(a,b){b?a.fadeOut(600,function(){$(this).remove()}):a.hide().remove()};this.clear_messages=function(){if(this.is_framed())return parent.rcmail.clear_messages();var a,b,d=this.messages;for(a in d)for(b in d[a].elements)d[a].obj&&
this.hide_message_object(d[a].obj);this.messages={}};this.show_popup_dialog=function(a,b,d,e){if(this.is_framed())return parent.rcmail.show_popup_dialog(a,b,d);a=$('<div class="popup">').html(a).dialog($.extend({title:b,buttons:d,modal:!0,resizable:!0,width:500,close:function(a,b){$(this).remove()}},e||{}));e=$(window);b=e.width();e=e.height();var f=a.width(),g=a.height();a.dialog("option",{height:Math.min(e-40,g+75+(d?50:0)),width:Math.min(b-20,f+20)});return a};this.set_page_buttons=function(){this.enable_command("nextpage",
"lastpage",this.env.pagecount>this.env.current_page);this.enable_command("previouspage","firstpage",1<this.env.current_page)};this.select_folder=function(a,b,d){this.treelist?this.treelist.select(a):this.gui_objects.folderlist&&($("li.selected",this.gui_objects.folderlist).removeClass("selected").addClass("unfocused"),$(this.get_folder_li(a,b,d)).removeClass("unfocused").addClass("selected"),this.triggerEvent("selectfolder",{folder:a,prefix:b}))};this.mark_folder=function(a,b,d,e){$(this.get_folder_li(a,
d,e)).addClass(b);this.triggerEvent("markfolder",{folder:a,mark:b,status:!0})};this.unmark_folder=function(a,b,d,e){$(this.get_folder_li(a,d,e)).removeClass(b);this.triggerEvent("markfolder",{folder:a,mark:b,status:!1})};this.get_folder_li=function(a,b,d){b||(b="rcmli");if(this.gui_objects.folderlist)return a=this.html_identifier(a,d),document.getElementById(b+a)};this.set_message_coltypes=function(a,b,d){var e=this.message_list,f=e?e.thead:null,g,h,k;this.env.coltypes=a;if(f){if(b){g=document.createElement("thead");
k=document.createElement("tr");c=0;for(h=b.length;c<h;c++)a=document.createElement("td"),a.innerHTML=b[c].html||"",b[c].id&&(a.id=b[c].id),b[c].className&&(a.className=b[c].className),k.appendChild(a);g.appendChild(k);f.parentNode.replaceChild(g,f);e.thead=f=g}g=0;for(h=this.env.coltypes.length;g<h;g++)b=this.env.coltypes[g],!(a=f.rows[0].cells[g])||"from"!=b&&"to"!=b&&"fromto"!=b||(a.id="rcm"+b,$("span,a",a).text(this.get_label("fromto"==b?d:b)),$("a",a).click(function(){return rcmail.command("sort",
this.id.replace(/^rcm/,""),this)}))}this.env.subject_col=null;this.env.flagged_col=null;this.env.status_col=null;0<=(g=$.inArray("subject",this.env.coltypes))&&(this.env.subject_col=g,e&&(e.subject_col=g));0<=(g=$.inArray("flag",this.env.coltypes))&&(this.env.flagged_col=g);0<=(g=$.inArray("status",this.env.coltypes))&&(this.env.status_col=g);e&&e.init_header()};this.set_rowcount=function(a,b){if(b&&b!=this.env.mailbox)return!1;$(this.gui_objects.countdisplay).html(a);this.set_page_buttons()};this.set_mailboxname=
function(a){this.gui_objects.mailboxname&&a&&(this.gui_objects.mailboxname.innerHTML=a)};this.set_quota=function(a){this.gui_objects.quotadisplay&&a&&"text"==a.type&&$(this.gui_objects.quotadisplay).html(a.percent+"%").attr("title",a.title);this.triggerEvent("setquota",a);this.env.quota_content=a};this.set_trash_count=function(a){this[(a?"un":"")+"mark_folder"](this.env.trash_mailbox,"empty","",!0)};this.set_unread_count=function(a,b,d,e){if(!this.gui_objects.mailboxlist)return!1;this.env.unread_counts[a]=
b;this.set_unread_count_display(a,d);e?this.mark_folder(a,e,"",!0):b||this.unmark_folder(a,"recent","",!0)};this.set_unread_count_display=function(a,b){var d,e,f,g,h;if(f=this.get_folder_li(a,"",!0)){g=this.env.unread_counts[a]?this.env.unread_counts[a]:0;e=$(f).children("a").eq(0);d=e.children("span.unreadcount");!d.length&&g&&(d=$("<span>").addClass("unreadcount").appendTo(e));e=0;if((h=f.getElementsByTagName("div")[0])&&h.className.match(/collapsed/))for(var k in this.env.unread_counts)k.startsWith(a+
...
"loading");this.http_post("headers",{_uid:this.env.uid},d)}};this.hide_headers=function(a,b){this.gui_objects.all_headers_row&&this.gui_objects.all_headers_box&&($(b).removeClass("hide-headers").addClass("show-headers"),$(this.gui_objects.all_headers_row).hide(),b.onclick=function(){rcmail.command("show-headers","",b)})};this.folder_selector=function(a,b){var d=this.folder_selector_element;if(!d){var e=[],f=this.env.delimiter,g=$('<ul class="toolbarmenu iconized">'),h=document.createElement("li"),
k=document.createElement("a"),q=document.createElement("span"),d=$('<div id="folder-selector" class="popupmenu"></div>');k.href="#";k.className="icon";$.each(this.env.mailboxes_list,function(){var a,b=a=0,g=l.env.mailboxes[this],m=g.id,s=k.cloneNode(!1),w=h.cloneNode(!1);g.virtual?s.className+=" virtual":(s.className+=" active",s.onclick=function(){d.hide().data("callback")(g.id)});g["class"]&&(s.className+=" "+g["class"]);for(;0<=(b=m.indexOf(f,b));)a++,b++;s.style.paddingLeft=a?16*a+"px":0;a=q.cloneNode(!1);
$(a).text(g.name);s.appendChild(a);w.appendChild(s);e.push(w)});g.append(e).appendTo(d);d.css({left:"-1000px",top:"-1000px"}).appendTo($("body")).show();10<e.length&&d.css("max-height",10*$("li",d)[0].offsetHeight+9);var m=function(a){a.target!=d.get(0)&&d.hide()};$(document.body).on("mouseup",m);$("iframe").contents().on("mouseup",m).load(function(a){try{$(this).contents().on("mouseup",m)}catch(b){}});this.folder_selector_element=d}this.element_position(d,a);d.show().data("callback",b)};this.element_position=
function(a,b){b=$(b);var d=$(window),e=b.width(),f=b.height(),g=d.height(),h=$(a).height(),k=$(a).width(),l=b.offset(),m=l.top,l=l.left+e;m+h>g&&(m-=h-f,0>m&&(m=Math.max(0,(g-h)/2)));l+k>d.width()&&(l-=k+e);a.css({left:l+"px",top:m+"px"})};this.html2plain=function(a,b){var d=this,e=this.set_busy(!0,"converting");this.log("HTTP POST: ?_task=utils&_action=html2text");$.ajax({type:"POST",url:"?_task=utils&_action=html2text",data:a,contentType:"application/octet-stream",error:function(a,b,h){d.http_error(a,
b,h,e)},success:function(a){d.set_busy(!1,null,e);$("#"+b).val(a);d.log(a)}})};this.plain2html=function(a,b){var d=this.set_busy(!0,"converting");a=a.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;");$("#"+b).val(a?"<pre>"+a+"</pre>":"");this.set_busy(!1,null,d)};this.url=function(a,b){var d="string"===typeof b?"&"+b:"";"string"!==typeof a?b=a:b&&"object"===typeof b||(b={});b._action=a?a:this.env.action;var e=this.env.comm_path,f,g={};b._action.match(/([a-z0-9_-]+)\/([a-z0-9-_.]+)/)&&
(b._action=RegExp.$2,e=e.replace(/\_task=[a-z0-9_-]+/,"_task="+RegExp.$1));for(f in b)void 0!==b[f]&&null!==b[f]&&(g[f]=b[f]);return e+"&"+$.param(g)+d};this.redirect=function(a,b){(b||null===b)&&this.set_busy(!0);this.is_framed()?parent.rcmail.redirect(a,b):(this.env.extwin&&("string"==typeof a?a+=(0>a.indexOf("?")?"?":"&")+"_extwin=1":a._extwin=1),this.location_href(a,window))};this.goto_url=function(a,b,d){this.redirect(this.url(a,b))};this.location_href=function(a,b,d){d&&this.lock_frame();"object"==
...
h){l.http_error(b,e,h,d,a)}})};this.http_post=function(a,b,d){var e=this.url(a);b&&"object"===typeof b?(b._remote=1,b._unlock=d?d:0):b+=(b?"&":"")+"_remote=1"+(d?"&_unlock="+d:"");var f=this.triggerEvent("request"+a,b);if(void 0!==f){if(!1===f)return!1;b=f}this.log("HTTP POST: "+e);this.start_keepalive();return $.ajax({type:"POST",url:e,data:b,dataType:"json",success:function(a){l.http_response(a)},error:function(b,e,f){l.http_error(b,e,f,d,a)}})};this.abort_request=function(a){a.request&&a.request.abort();
a.lock&&this.set_busy(!1,null,a.lock)};this.http_response=function(a){if(a){a.unlock&&this.set_busy(!1);this.triggerEvent("responsebefore",{response:a});this.triggerEvent("responsebefore"+a.action,{response:a});a.env&&this.set_env(a.env);if("object"===typeof a.texts)for(var b in a.texts)"string"===typeof a.texts[b]&&this.add_label(b,a.texts[b]);a.exec&&(this.log(a.exec),eval(a.exec));if(a.callbacks&&a.callbacks.length)for(b=0;b<a.callbacks.length;b++)this.triggerEvent(a.callbacks[b][0],a.callbacks[b][1]);
...
"collapse-all",!1)),this.message_list&&this.triggerEvent("listupdate",{folder:this.env.mailbox,rowcount:this.message_list.rowcount}));break;case "refresh":case "check-recent":$.each(this.env.recent_flags||{},function(a,b){l.set_message(a,"deleted",b.deleted);l.set_message(a,"replied",b.answered);l.set_message(a,"unread",!b.seen);l.set_message(a,"forwarded",b.forwarded);l.set_message(a,"flagged",b.flagged)}),delete this.env.recent_flags;case "getunread":case "search":this.env.qsearch=null;case "list":"mail"==
this.task?(this.enable_command("show","select-all","select-none",0<this.env.messagecount),this.enable_command("expunge",this.env.exists),this.enable_command("purge",this.purge_mailbox_test()),this.enable_command("expand-all","expand-unread","collapse-all",this.env.threading&&this.env.messagecount),"list"!=a.action&&"search"!=a.action||!this.message_list||(this.msglist_select(this.message_list),this.triggerEvent("listupdate",{folder:this.env.mailbox,rowcount:this.message_list.rowcount}))):"addressbook"==
...
this.mailto_handler_uri())?$(b).addClass("disabled"):$(b).click(function(){rcmail.register_protocol_handler(a);return!1})};this.browser_capabilities_check=function(){this.env.browser_capabilities||(this.env.browser_capabilities={});void 0===this.env.browser_capabilities.pdf&&(this.env.browser_capabilities.pdf=this.pdf_support_check());void 0===this.env.browser_capabilities.flash&&(this.env.browser_capabilities.flash=this.flash_support_check());void 0===this.env.browser_capabilities.tif&&this.tif_support_check()};
this.browser_capabilities=function(){if(!this.env.browser_capabilities)return"";var a,b=[];for(a in this.env.browser_capabilities)b.push(a+"="+this.env.browser_capabilities[a]);return b.join()};this.tif_support_check=function(){var a=new Image;a.onload=function(){rcmail.env.browser_capabilities.tif=1};a.onerror=function(){rcmail.env.browser_capabilities.tif=0};a.src="program/resources/blank.tif"};this.pdf_support_check=function(){var a=navigator.mimeTypes?navigator.mimeTypes["application/pdf"]:{},
b=navigator.plugins,d=b.length,e=/Adobe Reader|PDF|Acrobat/i;if(a&&a.enabledPlugin)return 1;if(window.ActiveXObject){try{if(axObj=new ActiveXObject("AcroPDF.PDF"))return 1}catch(f){}try{if(axObj=new ActiveXObject("PDF.PdfCtrl"))return 1}catch(g){}}for(i=0;i<d;i++)if(a=b[i],"String"===typeof a){if(e.test(a))return 1}else if(a.name&&e.test(a.name))return 1;return 0};this.flash_support_check=function(){var a=navigator.mimeTypes?navigator.mimeTypes["application/x-shockwave-flash"]:{};if(a&&a.enabledPlugin)return 1;
if(window.ActiveXObject)try{if(axObj=new ActiveXObject("ShockwaveFlash.ShockwaveFlash"))return 1}catch(b){}return 0};this.set_cookie=function(a,b,d){setCookie(a,b,d,this.env.cookie_path,this.env.cookie_domain,this.env.cookie_secure)};this.get_local_storage_prefix=function(){this.local_storage_prefix||(this.local_storage_prefix="roundcube."+(this.env.user_id||"anonymous")+".");return this.local_storage_prefix};this.local_storage_get_item=function(a,b,d){a=localStorage.getItem(this.get_local_storage_prefix()+
a);return null!==a?JSON.parse(a):b||null};this.local_storage_set_item=function(a,b,d){return localStorage.setItem(this.get_local_storage_prefix()+a,JSON.stringify(b))};this.local_storage_remove_item=function(a){return localStorage.removeItem(this.get_local_storage_prefix()+a)}}rcube_webmail.long_subject_title=function(l,x){if(!l.title){var a=$(l);a.width()+15*x>a.parent().width()&&(l.title=a.text())}};
